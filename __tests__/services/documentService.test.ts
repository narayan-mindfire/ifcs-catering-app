...
describe("documentService", () => {
  ...
  it("getFolders should fetch root folders by default", async () => {
    ...
    expect(apiClient.get).toHaveBeenCalledWith("/documents/folders", expect.objectContaining({
  ...
  it("getFolderFiles should fetch files for folder", async () => {
    ...
    expect(apiClient.get).toHaveBeenCalledWith("/documents/folders/f1/files", expect.any(Object));
  });

  it("getDownloadUrl should return url from response", async () => {
    (apiClient.get as jest.Mock).mockResolvedValue({ data: { success: true, data: { url: "http://test.com" } } });
