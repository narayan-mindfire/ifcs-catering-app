...
describe("deliveryService", () => {
  ...
  it("addSignature should clean signature and post data", async () => {
    ...
    (apiClient.post as jest.Mock).mockResolvedValue({ data: { success: true, data: { id: "d1" } } });

    await deliveryService.addSignature(flightId, "d1", "driver", signature, "Test comment");
