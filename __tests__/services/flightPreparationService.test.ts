...
describe("flightPreparationService", () => {
  ...
  it("getPreparations should return items on success", async () => {
    ...
    expect(apiClient.get).toHaveBeenCalledWith(
      expect.stringContaining("preparations"),
      expect.any(Object),
    );
  });
  ...
  it("updatePreparationFlag should call patch", async () => {
    ...
    const result = await flightPreparationService.updatePreparationFlag(
      flightId,
      "p1",
      payload as any,
    );
  ...
  it("getUserSignatures should handle both array and object responses", async () => {
    ...
    const result = await flightPreparationService.getUserSignatures(
      flightId,
      "u1",
    );
